...
void Game::shuffleDeck() {
    ...
    std::map<QString, Card*> shufflingCards;  // vector< pair<qstring, card>* >
    for (auto& pair : CardLibrary::cards) {
        shufflingCards[pair.first] = &pair.second;
    }
    ...
    for (const auto& card : shufflingCards) {
        deck.push(card.second);
    }
    qDebug() << "Top of deck:" << deck.top()->toString();
    // Card order for debugging
    std::vector<QString> cardOrder;
    for (auto& card : shufflingCards) {
        cardOrder.push_back(card.second->toString());
    }
...
void Game::chipsUpdated(int playerIndex, int newAmount) {
